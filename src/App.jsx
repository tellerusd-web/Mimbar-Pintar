...
import { Loader2, Key, AlertCircle, BookOpen, ShieldCheck } from 'lucide-react';
...
export default function App() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setError("Error: " + err.code); // Menampilkan kode error asli dari Firebase
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-emerald-50">
        <div className="text-center p-8 bg-white rounded-2xl shadow-xl">
          ...
          <button onClick={() => auth.signOut()} className="mt-4 text-red-600 underline">Keluar</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-100 p-4">
      <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-sm">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button 
            disabled={loading}
            className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-lg font-bold transition-all disabled:bg-slate-400"
          >
